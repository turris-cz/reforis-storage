...
from .utils import get_mocked_storage_client


def test_get_example(app):
    backend_response = {'key': 'value'}
    with get_mocked_storage_client(app, backend_response) as client:
        response = client.get('/storage/api/example')
...
def test_post_example_invalid_json(app):
    ...
    with get_mocked_storage_client(app, backend_response) as client:
        response = client.post('/storage/api/example')
...
def test_post_example_backend_error(app):
    backend_response = {'key': 'value'}
    with get_mocked_storage_client(app, backend_response) as client:
        response = client.post('/storage/api/example', json={'modules': []})
