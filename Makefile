...
PROJECT="reForis Storage Plugin"
...
.PHONY: install
install:
	opkg update && opkg install foris-controller-storage-module
	$(PYTHON) -m pip install -e .
	ln -sf /tmp/reforis-storage/reforis_static/reforis_storage /tmp/reforis/reforis_static/
...
.PHONY: lint-web
lint-web: venv
	$(VENV_BIN)/$(PYTHON) -m pylint --rcfile=pylintrc reforis_storage
	$(VENV_BIN)/$(PYTHON) -m pycodestyle --config=pycodestyle reforis_storage
...
create-messages: venv
	$(VENV_BIN)/pybabel extract -F babel.cfg -o ./reforis_storage/translations/messages.pot . --project=$(PROJECT) --version=$(VERSION) --copyright-holder=$(COPYRIGHT_HOLDER) --msgid-bugs-address=$(MSGID_BUGS_ADDRESS)

.PHONY: update-messages
update-messages: venv
	$(VENV_BIN)/pybabel update -i ./reforis_storage/translations/messages.pot -d ./reforis_storage/translations --update-header-comment
...
clean:
	...
	rm -rf $(JS_DIR)/node_modules/ reforis_static/reforis_storage/js/app.min.js
	$(PYTHON) -m pip uninstall -y reforis_storage
