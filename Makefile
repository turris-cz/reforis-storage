...
lint-web: venv
	$(VENV_BIN)/$(DEV_PYTHON) -m pylint --rcfile=pylintrc reforis_storage
	$(VENV_BIN)/$(DEV_PYTHON) -m pycodestyle --config=pycodestyle reforis_storage
...
create-messages:
	$(VENV_BIN)/pybabel extract -F babel.cfg -o ./reforis_storage/translations/messages.pot .
init-langs: create-messages
	for lang in $(LANGS); do \
		$(VENV_BIN)/pybabel init \
		-i reforis_storage/translations/messages.pot \
...
clean:
	...
	$(ROUTER_PYTHON) -m pip uninstall -y reforis_storage
